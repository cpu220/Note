...
lisi.sex; // 访问person.sex，这个查找链路就是原型链
...
在函数原型中加入成员，可以增强其对象的功能，但会导致原型污染。
...
JavaScript的原型概念源于它是一种基于原型的编程语言。虽然ES6引入了类语法，但底层实现仍然基于原型机制
...
// step 3
// 将dog的原型设置为Cat.prototype
Object.setPrototypeOf(dog, Cat.prototype);
dog.speak();  // Spot meows.
// 将cat的原型设置为Dog.prototype
Object.setPrototypeOf(cat, Dog.prototype);
cat.speak(); // Fluffy barks.
...
原型对象-> constructor->构造函数 -> __proto__ -> Function.prototype -> __proto__ -> Object.prototype -> __proto__ -> null
