...
React在v15以前的架构，被称之为stack架构。由于在虚拟DOM树的对比中，采用的是是递归，计算会消耗大量的时间，而且递归是无法被打断的。
所以从v16开始，重新调整了整体的架构，鑫的架构被称之为Fiber架构，而新的架构相比旧的架构，有一个最大的特点，就是可以进行时间切片。
...
2. 当进行IO操作的时候，需要等待数据返回，再进行后续的操作。这个等待的过程中，无法**快速响应**，这个实际上就是来自I/O的平经济。
...
当发现一帧（16ms）时间已经不够，不能苟在继续执行JS，需要渲染下一帧的时候，这个时候就会打断JS的执行，渲染下一帧。渲染完成后，再接着回来完成上一次没有执行完的计算。


### 解决I/O瓶颈
在React V16这个版本中，引入了Scheduler（调度器），用来调度任务的优先级。在wookloop中，每次都会调用 **shouldYield** 来判断当前是否有足够的剩余时间，如果不够，就暂停 Reconciler的执行，将主线程还给渲染线程，进行下一帧的渲染操作。
...
1. Render阶段，根据虚拟DOM，计算出要 **最终** 渲染的虚拟DOM
   - Scheduler : 调度器
   - Reconciler ： 协调器
2. commit阶段： 根据上一步计算出来的虚拟DOM，渲染具体的UI
   - Renderer： 渲染器


## 流程总结
当用户点击了按钮，首先Scheduler进行任务协调
...
2. 可以服用已完成的工作，或者丢弃不需要的工作 （在Diff中进行，具体是2次遍历）
...
无论是这三个哪一个，最终都是执行 performWorkUntilDeadline 方法，该方法的作用就是根据条件，调用  **flushWork**，并返回一个Boole，来判断是否有人物需要做，如果有，就继续调用 **performWorkUntilDeadline** 直到没有任务为止。
...
这里需要注意的事 workloop，内部会根据 advanceTimers 、shouldYieldToHost 来对当前进行的任务进行优先级调整。
    - 比如，如果timerQueue中有任务过去，则将任务插入到taskQueue中。
...
>在之前的版本中，对于当前Fiber元素伤挂载的deletions，会进行unMount处理。但在现在的版本中，是抽象为 **commitPassiveUnmountEffects**，放在**ReactFiberWorkLoop** 中调用。
